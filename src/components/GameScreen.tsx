...
export const GameScreen: React.FC<GameScreenProps> = ({
  ...
  // Start background ambient music if enabled when game screen is active
  useEffect(() => {
    if (ambientMusic.isEnabled() && !ambientMusic.isCurrentlyPlaying()) {
      ambientMusic.start();
    }
  }, []);

  // Sync Audio, Music & Voice subscriptions
  useEffect(() => {
    const unsubSound = soundFx.subscribe((en) => setSoundOn(en));
    const unsubMusic = ambientMusic.subscribe((en) => setMusicOn(en));
  ...
  // Initialize or reset question timer
  useEffect(() => {
    speechEngine.stop();
  ...
  const handleSelectOption = (optionIndex: number) => {
    ...
    // 1. Sonido sutil al seleccionar una respuesta
    ...
    if (isCorrect) {
      ...
      // Check level up / streak milestone
      if (newStreak === 3) {
        streakBonus = 50;
        bonusLabel = ' ¡Racha de 3 (+50)! 🔥';
        soundFx.playLevelUp();
        speechEngine.speakLevelUp(3);
        setLevelUpNotice('¡Nivel de Racha Desbloqueado! Racha x3');
        setTimeout(() => setLevelUpNotice(null), 3000);
      } else if (newStreak === 5) {
        streakBonus = 100;
        bonusLabel = ' ¡Super Racha de 5 (+100)! ⚡';
        soundFx.playLevelUp();
        speechEngine.speakLevelUp(5);
        setLevelUpNotice('¡Subiste al Nivel Maestro! Racha x5');
        setTimeout(() => setLevelUpNotice(null), 3500);
      } else if (newStreak > 5 && newStreak % 3 === 0) {
        streakBonus = 50;
        bonusLabel = ` ¡Racha x${newStreak}! 🔥`;
        soundFx.playLevelUp();
        speechEngine.speakLevelUp(newStreak);
      } else {
        // Regular correct announcement by the host
        speechEngine.speakCorrect(newStreak, Math.round((100 + speedBonus) * levelMultiplier));
      }
      ...
      setFloatingPoints({ amount: totalEarned, bonus: bonusLabel.trim() });
      setScorePopped(true);
      setTimeout(() => setScorePopped(false), 400);
      ...
      // Decir respuesta incorrecta de forma motivadora y breve
      const correctOptionText = currentQ ? currentQ.options[currentQ.correctIndex] : undefined;
  ...
  const handleNextQuestion = () => {
    ...
    if (currentIndex < questions.length - 1) {
      ...
      // Finish game: stop ambient music to highlight results
      ambientMusic.stop();

      const totalTime = responseTimes.reduce((a, b) => a + b, 0);
      const avgTime = responseTimes.length
        ? Number((totalTime / responseTimes.length).toFixed(1))
        : 0;
  ...
  return (
    <div className="w-full max-w-xl mx-auto flex flex-col gap-3 sm:gap-4 py-1 sm:py-2 relative">
      {/* Level Up Notification Badge */}
      {levelUpNotice && (
        <div className="animate-fade-in absolute -top-3 left-1/2 -translate-x-1/2 z-30 px-4 py-1.5 rounded-full bg-gradient-to-r from-amber-400 to-yellow-500 text-slate-950 font-black text-xs shadow-xl shadow-amber-500/30 flex items-center gap-1.5 border border-white/30 whitespace-nowrap">
      ...
      <div className="bg-slate-800/80 border border-slate-700/80 rounded-2xl p-3 sm:p-4 backdrop-blur-md shadow-lg flex items-center justify-between gap-2">
        ...
        {/* 15s Timer Display */}
        <div
          className={`flex items-center gap-1.5 px-3 py-1 rounded-full border font-black text-xs sm:text-sm transition-colors ${getTimerStyles()}`}
        >
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-orange-500/15 border border-orange-500/30 text-orange-400 font-black text-xs sm:text-sm">
          <Flame
            className={`w-3.5 h-3.5 fill-orange-400 transition-transform ${
              streak >= 3 ? 'scale-125 animate-bounce' : ''
            }`}
          />
          <span>{streak}</span>
        </div>

        {/* Points Display with Pop Micro-animation */}
        <div className="flex flex-col items-end relative">
          <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-400">
            Puntos
          </span>
          <span
            className={`text-sm sm:text-base font-black text-amber-400 flex items-center gap-1 transition-transform ${
              scorePopped ? 'animate-gentle-pop text-yellow-300' : ''
            }`}
          >
          ...
          {floatingPoints && (
            <div className="absolute -top-5 right-0 animate-float-points pointer-events-none text-xs font-black text-amber-300 bg-amber-400/20 border border-amber-400/40 px-2 py-0.5 rounded-full whitespace-nowrap shadow-lg">
      ...
      {/* Quick Audio, Music & Voice Controls Bar */}
      <div className="flex items-center justify-between px-1 text-xs gap-1.5 flex-wrap">
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1 rounded-lg border text-[11px] font-semibold transition-all hover:-translate-y-0.5 active:translate-y-0 ${
              soundOn
          ...
          <button
            ...
            title="Música ambiental de concurso"
          >
        ...
        <div className="flex items-center gap-1.5">
          {/* Stop speech button if currently speaking */}
          {isSpeaking && (
            <button
              id="btn-stop-speech"
              type="button"
              onClick={() => speechEngine.stop()}
              className="flex items-center gap-1 px-2 py-1 rounded-lg bg-rose-950/70 border border-rose-700 text-rose-300 text-[11px] font-bold animate-pulse"
              title="Detener voz"
            >
          ...
          {/* Quick Settings modal button */}
          <button
            id="btn-voice-settings-game"
            type="button"
            onClick={() => setIsVoiceModalOpen(true)}
            className="flex items-center gap-1 px-2 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 hover:text-white text-[11px] font-semibold transition-all hover:-translate-y-0.5"
            title="Ajustes de audio, música y voz"
          >
      ...
      {/* Main Question Card with Smooth Transition Micro-animation */}
      <div
        key={currentIndex}
        className="animate-slide-up relative overflow-hidden bg-slate-900/90 border border-slate-700/80 rounded-3xl p-5 sm:p-7 shadow-2xl backdrop-blur-xl"
      >
        ...
        {/* On-Demand Voice Buttons: "🔊 Escuchar pregunta" & "📋 Escuchar opciones" */}
        <div className="flex flex-wrap items-center gap-2 mb-5">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-cyan-950/60 hover:bg-cyan-900/70 border border-cyan-600/60 text-cyan-200 font-bold text-xs transition-all hover:-translate-y-0.5 active:translate-y-0 active:scale-95 shadow-sm shadow-cyan-950"
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-750 border border-slate-700 text-slate-200 font-bold text-xs transition-all hover:-translate-y-0.5 active:translate-y-0 active:scale-95"
          ...
          {isSpeaking && (
            <span className="text-[11px] text-cyan-400 font-semibold flex items-center gap-1.5 px-2 py-1 rounded-lg bg-cyan-950/40 border border-cyan-800/40">
              <span className="w-2 h-2 rounded-full bg-cyan-400 animate-ping" />
              <span>Presentador hablando...</span>
            </span>
          )}
        </div>

        {/* 4 Multiple Choice Options with Micro-animations */}
        <div className="grid grid-cols-1 gap-3 mb-4">
          {currentQ.options.map((optionText, optIndex) => {
            ...
            let buttonStyles =
              'bg-slate-800/70 border-slate-700/80 text-slate-100 hover:bg-slate-750 hover:border-slate-500 hover:-translate-y-0.5 hover:shadow-lg';
            ...
            if (isAnswered) {
              if (isThisCorrect) {
                buttonStyles =
                  'bg-emerald-950/80 border-emerald-400 text-emerald-100 ring-2 ring-emerald-400/60 shadow-lg shadow-emerald-500/20';
                badgeStyles = 'bg-emerald-400 text-slate-950 font-black';
                animationClass = 'animate-halo-glow';
              } else if (isThisSelected && !isThisCorrect) {
                buttonStyles =
                  'bg-rose-950/80 border-rose-500 text-rose-100 ring-2 ring-rose-500/60 shadow-lg shadow-rose-500/20';
                badgeStyles = 'bg-rose-500 text-white font-black';
                animationClass = 'animate-soft-shake';
              } else {
            ...
            return (
              <button
                ...
                className={`w-full text-left p-4 rounded-2xl border font-bold text-sm sm:text-base transition-all duration-200 flex items-center justify-between gap-3 shadow-md active:scale-[0.99] ${buttonStyles} ${animationClass}`}
              >
        ...
        {isAnswered && (
          <div className="mt-4 pt-4 border-t border-slate-800 flex flex-col gap-4 animate-fade-in">
            ...
            <button
              ...
              className="w-full py-4 px-6 rounded-2xl bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-slate-950 font-black text-base sm:text-lg tracking-wide shadow-lg shadow-amber-500/20 active:scale-[0.98] hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2"
            >
              <span>
                {currentIndex === questions.length - 1
                  ? 'VER RESULTADOS DE LA PARTIDA'
                  : 'SIGUIENTE PREGUNTA'}
              </span>
      ...
      <div className="text-center pt-1">
        <button
          ...
          onClick={() => {
            if (
              window.confirm(
                '¿Seguro que quieres salir de esta partida? Tu progreso actual se perderá.'
              )
            ) {
              soundFx.playClick();
              speechEngine.stop();
      ...
      {/* Audio, Music & Voice Settings Modal */}
      <VoiceSettingsModal
